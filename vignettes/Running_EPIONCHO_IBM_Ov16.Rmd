...
remotes::install_github("mrc-ide/EPIONCHO.IBM", ref = "ov16")
...
The entire vignette should take ~30 minutes to run, with most of the time taken up by computational time. As a note, the code below will only show the general process for Gabon and Togo. 
For the full R script for Gabon, use `all_funcs_combined.R` in the top level folder. For the full script for togo, use `all_funcs_combined_togo.R` in the top level folder.
...
library(dplyr)
processOutputFiles <- function (files='test_output_folder/test_mfp_abr_output_folder/', verbose=TRUE, onlyCalcMFP=FALSE, useSerorevert=FALSE, onlyCalcOv16Trends=FALSE) {
...
H1) L3 Exposure
H2) During L4-L5 Moult
H3) In the presence of Any Adult Worm
H4) With a mating worm pair and any (modelled) microfilarae
H5) With a mating worm pair (not in paper)
H6) With a mating worm pair and detectable (via skin-snips) microfilarae (not in paper)
...
All 3 seroreversion hypotheses are run by default. 
...
7 - OV16 Seroprevalence (Hypothesis 5) (not in paper)
8 - OV16 Seroprevalence (Hypothesis 6) (not in paper)
...
# ov16_pos_mating_no_mf - ov16 status for hypothesis 5 (not in paper)
# ov16_pos_mating_detectable_mf - ov16 status for hypothesis 6 (not in paper)
...
You can look at the time trend of Ov16 seroprevalence as well, however it is only output with the best fit hypothesis, and without diagnostic adjustment (Hypothesis 4, no seroreversion and H4 w/ finite seroreversion). 
...
cstm_treat_params <- list(start_biannual=treat.strt.yrs+12, coverage_changes=c(treat.strt.yrs+5, treat.strt.yrs+10), coverage_change_values=c(0.65, 0.75, 0.80))
...
dir.create(file.path("test_output_folder/test_ov16_mda_output_folder_togo/"), recursive=TRUE, showWarnings = FALSE)
saveRDS(output, paste("test_output_folder/test_ov16_mda_output_folder_togo/ov16_output_togo_", prefecture, "_", kE,".rds", sep=""))
```
...
library(tidyr)
# process data
seroprevalence_data_mda <- processOutputFiles(file="test_output_folder/test_ov16_mda_output_folder_togo/")$allOutputs
...
library(dplyr)
library(ggplot2)
sero_trend_mda_all <- processOutputFiles(file="test_output_folder/test_ov16_mda_output_folder_togo/", onlyCalcOv16Trends=TRUE)
sero_trend_mda_no_revert <- as.data.frame(sero_trend_mda_all$ov16_trend_df) %>% group_by(Ke, run_num) %>% mutate(time = (row_number()/2)) %>% ungroup()
sero_trend_mda_finite_revert <- as.data.frame(sero_trend_mda_all$ov16_serorevert_trend_df) %>% group_by(Ke, run_num) %>% mutate(time = (row_number()/2)) %>% ungroup()

mfp_trend_mda <- as.data.frame(processOutputFiles(file="test_output_folder/test_ov16_mda_output_folder_togo/", onlyCalcMFP=TRUE)$mf_prev_df) %>% group_by(Ke, run_num) %>% mutate(time = row_number()) %>% ungroup()

time_plot_mda <- ggplot() + geom_line(
  data=mfp_trend_mda %>% group_by(time) %>% summarize(prev=mean(mf_prev), .groups="drop"),
  aes(x=time, y=prev*100, color="MF Prevalence")) + 
  geom_line(
    data=sero_trend_mda_no_revert %>% group_by(time) %>% summarize(prev=mean(ov16_vals), .groups="drop"),
    aes(x=time, y=prev*100, color="Ov16 Seroprevalence w/ No Seroreversion")
  ) +
  geom_line(
    data=sero_trend_mda_finite_revert %>% group_by(time) %>% summarize(prev=mean(ov16_vals), .groups="drop"),
    aes(x=time, y=prev*100, color="Ov16 Seroprevalence w/ Finite Seroreversion")
  ) +
