...
- name: Jacob Stapely
  ...
  role:         
  - Writing - Review & Editing
- name: Michelle E. Murdoch
  affiliation: West Hertfordshire Teaching Hospitals NHS Trust
  role:
  - Resources
  - Validation
  - Writing - Review & Editing
- name: Ian E. Murdoch
  affiliation: Ahmadu Bello University Hospital, Institute of Ophthalmology
  role:         
  - Resources
  - Validation
  - Writing - Review & Editing
- name: Gladys A. Ozoh
  ...
  - Resources
  - Writing - Review & Editing
- name: Jonathan F. Mosser
  affiliation: University of Washington, Institute for Health Metric and Evaluation
  role:         
  - Writing - Review & Editing
...
The library ggplot2 will also be used for this.

## 1.2 Run time

When run on laptop machine using Windows, a single run of EPIONCHO-IBM with the activated morbidity module takes ~30 minutes.
...
The first four elements (`mf_prev`, `mf_intens`, `ov16_seroprevalence_combined_seroreversion`, and `L3`) contain the temporal dynamics (at 1 day increments) for the microfilarial prevalence (individuals age > 5), the population mean microfilarial intensity (individuals age > 5), the temporal dynamics for seroprevalence (w/ seroreverstion), and the mean number of L3 larvae in the black fly population. To plot these over time, use: 
...
plot(years, output_equilibrium$ov16_seroprevalence_combined_seroreversion, type = 'l', xlab = 'time (years)', ylab = 'seroprevalence', ylim = c(0, 1))
...
plot(years, output_treat_annual$ov16_seroprevalence_combined_seroreversion, type = 'l', xlab = 'time (years)', ylab = 'seroprevalence', ylim = c(0, 1))
...
plot(years, output_treat_biannual$ov16_seroprevalence_combined_seroreversion, type = 'l', xlab = 'time (years)', ylab = 'seroprevalence', ylim = c(0, 1))
