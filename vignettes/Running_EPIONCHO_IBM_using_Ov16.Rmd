...
Use the following code to test the ABRs and k~E values that represent a hypoendemic area, with a microfilarial prevalence of around 8%. You can use the `prob_serorevert_fast` parameter to choose how quickly people's Ov16 status seroreverts, with a value of 1 assuming individuals serorevert as soon as the seroconversion stimulus is lost, and a value of 0 assuming individuals serorevert only upon complete infection clearence.
...
process_multiple_runs <- function (files='', morbidity_runs = FALSE, verbose=TRUE, ov16_indiv_location=1) {
  ...
  for (file in list.files(fileToUse)) {
    ...
    age <- ov16_indiv_matrix[, (ov16_indiv_location*6)-5]
    sex <- ifelse(ov16_indiv_matrix[,(ov16_indiv_location*6)-4] == 1, "Male", "Female")
    mf_status <- ov16_indiv_matrix[,(ov16_indiv_location*6)-3]
    mf_load <- ov16_indiv_matrix[,(ov16_indiv_location*6)-2]
    ov16_seroreversion_type <- ov16_indiv_matrix[,(ov16_indiv_location*6)-1]
    ov16_status <- ov16_indiv_matrix[,(ov16_indiv_location*6)]
    ...
    if(i == 1) {
      ...
      ov16_indiv_df <- data.frame(matrix(ncol=9, nrow=num_individuals*total_files))
      colnames(ov16_indiv_df) <- c("age", "sex", "ov16_seroreversion_type", "ov16_status", "mf_status", "mf_load", "ABR", "Ke", "run_num")
    }
    ...
    ov16_indiv_df[start_index:end_index,-9] <- list(age, sex, ov16_seroreversion_type, ov16_status, mf_status, mf_load, rep(tmpRDSData$ABR, num_individuals), rep(kE, num_individuals))
    ov16_indiv_df[start_index:end_index,9] <- i
  ...
  ov16_indiv_df <- ov16_indiv_df %>% as.data.frame() %>%
    mutate(
      age = as.numeric(age),
      mf_status = as.numeric(mf_status),
      mf_load = as.numeric(mf_load),
      ov16_status = as.numeric(ov16_status),
...
mfp_vs_abr_plot <- ggplot(data=gabon_mfp_data_df_mutated) + 
  geom_point(aes(x=factor(ABR), y=mean_mfp), stat="identity") +
...
This section will explain how to output Ov16 seroprevalence in the model. You can use the `prob_serorevert_fast` parameter to choose how quickly people's Ov16 status seroreverts, with a value of 1 assuming individuals serorevert as soon as the seroconversion stimulus is lost, and a value of 0 assuming individuals serorevert only upon complete infection clearence.
...
The main value of the `output` that will have the seroprevalence values is `ov16_indiv_matrix`. This is a matrix with N x (6*i) rows, with N being the number of individuals in the population. The number of columns is a multiple of 6, defined by i, where i is the number of output times in `ov16_store_times`.
The columns (by index) contain the following information for the given individual in that row:
Column:
1 - Age
2 - Sex (1 = Male)
3 - Skin Snip Result (positive or negative)
4 - Skin Snip Load
5 - Type of seroreversion used for a given individual
6 - Ov16 Seroprevalence with Seroreversion based upon parameter `prob_serorevert_fast`
...
seroprevalence_data_adj <- seroprevalence_data %>% group_by(run_num) %>%
  mutate(
          ov16_status=calcSensSpecSeroPrev(ov16_status, sens, spec, probs),
...
# plot the data
plot <- ggplot() +
    geom_line(aes(x=age_groups, y=ov16_prev*100), linewidth=1.1, data=seroprevalence_data_summary) +    
    theme_bw() +
    xlab("Age (years)") +
...
sero_trend <- as.data.frame(processed_no_mda_data$ov16_trends_df) %>%
              group_by(Ke, year) %>% summarize(ov16_prev=mean(ov16_seroprevalence_combined_seroreversion), .groups="drop")
...
time_plot <- ggplot() + geom_line(
  ...
  geom_line(
    data=sero_trend,
    aes(x=year, y=ov16_prev*100, color="Ov16 Seroprevalence")
  ) +
...
# plot the data
plot_mda <- ggplot() +
    geom_line(aes(x=age_groups, y=ov16_prev*100), linewidth=1.1, data=seroprevalence_data_mda_summary) +    
    theme_bw() +
    xlab("Age (years)") +
    ylab("Ov16 Seroprevalence (%)")
plot_mda
