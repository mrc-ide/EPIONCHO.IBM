# ========================================================================================================================== #
#                                 TESTING MORBIDITY (SKIN DISEASE) - 30/05/23                                                #
# ========================================================================================================================== #
...
# ========================================================================= #
#output_equilibrium_morbidity <- output_equilibrium_morbidity_mf80prev
...
modelled_mfcount_age_prop <- mf_age_df %>%
  group_by(age_cat) %>%
  summarise(Mean = mean(mf_count))

mf_means <- modelled_mfcount_age_prop$Mean


barplot(mf_means, xlab="age cat")

barplot(mf_means,
        xlab = "age cats",
        ylab = "mean mf count",
        names.arg = c("0-1", "2-4", "5-9", "10-19", "20-29", "30-49", "50-80"),
        col = "darkred")

# ===================================================================== #

age_to_samp_vec_nonreversible <- seq(0+1/366, 79+1/366, 1)
age_to_samp_vec_nonreversible <- round(age_to_samp_vec_nonreversible,6)

age_to_samp_vec_allages <- seq(0+1/366, 80, 1/366)
age_to_samp_vec_allages <- round(age_to_samp_vec_allages,6)

countOccurrences <- function(vector1, vector2) {
  counter <- 0
...
# Example usage
# vector1 <- c(1, 2, 3, 2, 4, 1)
# vector2 <- c(2, 1, 5, 2, 1, 2, 3, 2, 6)
occurrences <- countOccurrences(vector1 = age_to_samp_vec_nonreversible, vector2 = age_to_samp_vec_allages)
