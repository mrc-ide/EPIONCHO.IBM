...
old_folder = "_agg_data_200_years/"
folder = "_agg_data_final_worm_revert/"#"_agg_data_final/"
oti_folder_100 = "_agg_data_final_vc_100/"
oti_folder = "_agg_data_immediate_sero_vc_1993/"
immediate_sero_folder = "_agg_data_immediate_sero/"
oti_immediate_sero_folder = "_agg_data_immediate_sero_vc_1993/"
oti_gradual_sero_folder = "_agg_data_gradual_sero_vc_1993/"
oti_gradual_sero_folder_100_vc = "_agg_data_gradual_sero_vc_1993_100/"
run_version = "v7"
use_oti_100 <- FALSE
if (use_oti_100) {
...
summarizeSimulationDataHelper_2 <- function(df, groupByCols1, groupByCols2, hypNamesLoc, hypothesisNames, groupHyp) {
  ...
  if("age_groups" %in% groupByCols2) {
    ...
    #df1 <- df1 %>% dplyr::group_by(!!!syms(groupByCols2)) %>% dplyr::summarise(ov16_prev=mean(ov16_prev), mf_prev=mean(mf_prev), .groups="drop") %>% as.data.frame()
    #df2 <- df2 %>% dplyr::group_by(!!!syms(groupByCols2)) %>% dplyr::summarise(ov16_prev_lb=mean(ov16_prev_lb), mf_prev=mean(mf_prev), .groups="drop") %>% as.data.frame()
    #df3 <- df3 %>% dplyr::group_by(!!!syms(groupByCols2)) %>% dplyr::summarise(ov16_prev_ub=mean(ov16_prev_ub), mf_prev=mean(mf_prev), .groups="drop") %>% as.data.frame()
    ...
    df <- df1 %>% dplyr::group_by(!!!syms(groupByCols2)) %>%
    dplyr::summarise(
      ...
      mf_q1=quantile(mf_prev, probs=0.025),
      mf_q3=quantile(mf_prev, probs=0.975),
      mf_prev=mean(mf_prev), .groups="drop"
    ) %>% as.data.frame()
  }
  return(df)
}
...
```{r}
look_at_mfp = TRUE
look_at_time_trends = TRUE
all_places_to_pick <- c("_start_2015", "_mid_2015", "_end_2015")
if (look_at_mfp) {
  folder_for_oti_mf = folder
  if(use_oti_100) {
    folder_for_oti_mf = oti_folder_100
  }
  ...
  if (look_at_time_trends) {
    ...
    ) %>% filter(time < 220) %>% group_by(prefecture, vctr.ctrl.eff, time, Ke) %>% summarize(prev=mean(mf_prev), .groups="drop") %>% mutate(prev_type="MF Prevalence", sero="no serorevert")
    ...
    togo_sero_trend <- rbind(togo_bassar_sero_trend, togo_oti_sero_trend, togo_keran_sero_trend) %>% filter(time < 220) %>% group_by(prefecture, vctr.ctrl.eff, time, Ke, prev_type, sero) %>% summarize(prev=mean(ov16_vals), .groups="drop") %>% mutate(prefecture = tolower(prefecture))
    ...
    time_trend_plot_full <- ggplot() + 
      ...
      ylab("Prevalence (%)") +
      scale_color_manual("Vector Control Efficacy", values=c("brown", "red", "orange"), labels=c("60%", "75%", "95%")) +
  ...
  for(pick in all_places_to_pick) {
    ...
    if (use_oti_100) {
      combined_mfp_vc_100 <- selectRunsFromAllPrefectures(
        ...
        readRDS(paste("data/keran", folder, "togo_keran_100_mount_data", pick, ".RDS", sep=""))$allOutputs %>% mutate(vctr.ctrl.eff=ifelse(vctr.ctrl.eff == 0.95, 1, vctr.ctrl.eff)) %>% filter(vctr.ctrl.eff == 1),
        readRDS(paste("data/bassar", folder, "togo_bassar_100_mount_data", pick, ".RDS", sep=""))$allOutputs %>% mutate(vctr.ctrl.eff=ifelse(vctr.ctrl.eff == 0.95, 1, vctr.ctrl.eff)) %>% filter(vctr.ctrl.eff == 1),
        sampleNum=c(500*0.22,500*0.37,500*0.41),
        togoMFData
...
if (use_oti_100) {
  ...
  simulatedOti_k3_100_mount[
    simulatedOti_k3_100_mount$vctr.ctrl.eff == 0.95,
...
for (sens_spec_val in sens_spec_options) {
  k3_instant_seroreversion_simulation_summary_combined_sens_spec_list[[paste("bassar", sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedBassar_k3_instant_seroreversion, sens_spec_val, groupBySex=FALSE, summary=TRUE)
}
...
if (use_oti_100) {
  ...
  simulatedOti_k3_seroreversion_vc_100 <- togo_oti_seroreversion_agg_vc_100$allOutputs %>% filter(Ke == 0.3) %>% fix_age_groups()
  simulatedOti_k3_instant_seroreversion[
    simulatedOti_k3_instant_seroreversion$vctr.ctrl.eff == 0.95,
  ] = simulatedOti_k3_seroreversion_vc_100
  simulatedOti_k3_instant_seroreversion[
    simulatedOti_k3_instant_seroreversion$vctr.ctrl.eff == 1,
  "vctr.ctrl.eff"] = 0.95
}
for (sens_spec_val in sens_spec_options) {
  k3_instant_seroreversion_simulation_summary_combined_sens_spec_list[[paste("oti", sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedOti_k3_instant_seroreversion, sens_spec_val, groupBySex=FALSE, summary=TRUE)
}
...
for (sens_spec_val in sens_spec_options) {
  k3_instant_seroreversion_simulation_summary_combined_sens_spec_list[[paste("keran", sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedKeran_k3_instant_seroreversion, sens_spec_val, groupBySex=FALSE, summary=TRUE)
}
...
for (sens_spec_val in sens_spec_options) {
  k3_instant_seroreversion_simulation_summary_age_group_combined_sens_spec_list[[paste(sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedOti_k3_instant_seroreversion, sens_spec_val, groupBySex=FALSE)
}
...
if(use_k4) {
  ...
  for (sens_spec_val in sens_spec_options) {
    k4_instant_seroreversion_simulation_summary_combined_sens_spec_list[[paste("bassar", sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedBassar_k4_instant_seroreversion, sens_spec_val, groupBySex=FALSE, summary=TRUE)
  }
  ...
  if (use_oti_100) {
    ...
    simulatedOti_k4_instant_seroreversion[
      simulatedOti_k4_instant_seroreversion$vctr.ctrl.eff == 0.95,
  ...
  for (sens_spec_val in sens_spec_options) {
    k4_instant_seroreversion_simulation_summary_combined_sens_spec_list[[paste("oti", sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedOti_k4_instant_seroreversion, sens_spec_val, groupBySex=FALSE, summary=TRUE)
  }
  
  simulatedKeran_k4_instant_seroreversion <- togo_keran_instant_seroreversion_agg$allOutputs %>% filter(Ke == 0.4) %>% fix_age_groups()
  
  for (sens_spec_val in sens_spec_options) {
    k4_instant_seroreversion_simulation_summary_combined_sens_spec_list[[paste("keran", sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedKeran_k4_instant_seroreversion, sens_spec_val, groupBySex=FALSE, summary=TRUE)
  }
  ...
  combinedSimulation_k4_seroreversion <- selectRunsFromAllPrefectures(
    ...
    sampleNum=sample_num_ov16)
  for (sens_spec_val in sens_spec_options) {
    k4_instant_seroreversion_simulation_summary_age_group_combined_sens_spec_list[[paste(sens_spec_val, collapse="_")]] <- summarizeSimulationData(combinedSimulation_k4_seroreversion, sens_spec_val, groupBySex=FALSE)
  }
}
...
for (sens_spec_val in sens_spec_options) {
  k3_gradual_seroreversion_simulation_summary_combined_sens_spec_list[[paste("bassar", sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedBassar_k3_gradual_seroreversion, sens_spec_val, groupBySex=FALSE, summary=TRUE)
}
...
if (use_oti_100) {
  ...
  simulatedOti_k3_gradual_seroreversion[
    simulatedOti_k3_gradual_seroreversion$vctr.ctrl.eff == 0.95,
  ] = simulatedOti_k3_seroreversion_vc_100
  simulatedOti_k3_gradual_seroreversion[
...
for (sens_spec_val in sens_spec_options) {
  k3_gradual_seroreversion_simulation_summary_combined_sens_spec_list[[paste("oti", sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedOti_k3_gradual_seroreversion, sens_spec_val, groupBySex=FALSE, summary=TRUE)
}
...
for (sens_spec_val in sens_spec_options) {
  k3_gradual_seroreversion_simulation_summary_combined_sens_spec_list[[paste("keran", sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedKeran_k3_gradual_seroreversion, sens_spec_val, groupBySex=FALSE, summary=TRUE)
}
...
for (sens_spec_val in sens_spec_options) {
  k3_gradual_seroreversion_simulation_summary_age_group_combined_sens_spec_list[[paste(sens_spec_val, collapse="_")]] <- summarizeSimulationData(combinedSimulation_k3_finite_seroreversion, sens_spec_val, groupBySex=FALSE)
}
...
if(use_k4) {
  ...
  for (sens_spec_val in sens_spec_options) {
    k4_gradual_seroreversion_simulation_summary_combined_sens_spec_list[[paste("bassar", sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedBassar_k4_gradual_seroreversion, sens_spec_val, groupBySex=FALSE, summary=TRUE)
  }
  ...
  if (use_oti_100) {
    ...
    simulatedOti_k4_gradual_seroreversion[
      simulatedOti_k4_gradual_seroreversion$vctr.ctrl.eff == 0.95,
    ] = simulatedOti_k4_seroreversion_vc_100
    simulatedOti_k4_gradual_seroreversion[
  ...
  for (sens_spec_val in sens_spec_options) {
    k4_gradual_seroreversion_simulation_summary_combined_sens_spec_list[[paste("oti", sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedOti_k4_gradual_seroreversion, sens_spec_val, groupBySex=FALSE, summary=TRUE)
  }
  
  simulatedKeran_k4_gradual_seroreversion <- togo_keran_gradual_seroreversion_agg$allOutputs %>% filter(Ke == 0.4) %>% fix_age_groups()
  
  for (sens_spec_val in sens_spec_options) {
    k4_gradual_seroreversion_simulation_summary_combined_sens_spec_list[[paste("keran", sens_spec_val, collapse="_")]] <- summarizeSimulationData(simulatedKeran_k4_gradual_seroreversion, sens_spec_val, groupBySex=FALSE, summary=TRUE)
  }
  ...
  combinedSimulation_k4_seroreversion <- selectRunsFromAllPrefectures(
    ...
    sampleNum=sample_num_ov16)
  for (sens_spec_val in sens_spec_options) {
    k4_gradual_seroreversion_simulation_summary_combined_sens_spec_list[[paste(sens_spec_val, collapse="_")]] <- summarizeSimulationData(combinedSimulation_k4_seroreversion, sens_spec_val, groupBySex=FALSE)
  }
}
...
for(pick in all_places_to_pick) {
  ...
  if(length(k3_mfp_df) == 0) {
    k3_mfp_df <- summarizeSimulationData(combinedSimulation_k3, c(1,1), groupBySex=FALSE) %>% mutate(sample_time=pick)
  } else {
    k3_mfp_df <- rbind(k3_mfp_df,
                       summarizeSimulationData(combinedSimulation_k3, c(1,1), groupBySex=FALSE) %>% mutate(sample_time=pick))
  }
  gc()
}
  ...
  for(vct_ctr in vct_ctrl_list) {
    ...
    if(length(lsr_calculation) == 0) {
      lsr_calculation <- merge(tmp_projection_data, originalData, by=c("age_groups")) %>%
        mutate(lsr_value = (((ov16_prev*100) - (ov16_seroprev*100))**2) * pct_pop) %>%
...
if (use_old_LSRs) {
  ...
  k3_instant_seroreversion_lsrs <- calculateLSRs(togoOv16Data, k3_instant_seroreversion_simulation_summary_age_group_combined_sens_spec_list)
  
  k3_instant_seroreversion_lsrs %>% group_by(Hypothesis, vctr.ctrl.eff, sens, spec) %>% summarise(cum_lsr = sum(lsr_value), .groups="drop") %>% ungroup() %>% as.data.frame() %>% select(Hypothesis, vctr.ctrl.eff, sens, spec, cum_lsr) %>% pivot_wider(names_from = spec, values_from=c(cum_lsr)) %>% group_by(sens) %>% mutate(across(3:5, ~ ifelse(. == min(.), text_spec(round(., 2), bold=TRUE), round(., 2)))) %>% arrange(sens) %>% kable('html', escape=FALSE) %>% kable_classic() #%>% row_spec(c(9), extra_css="border-bottom: 1px solid;")
  
  k3_gradual_seroreversion_lsrs <- calculateLSRs(togoOv16Data, k3_gradual_seroreversion_simulation_summary_age_group_combined_sens_spec_list)
  
  k3_gradual_seroreversion_lsrs %>% group_by(Hypothesis, vctr.ctrl.eff, sens, spec) %>% summarise(cum_lsr = sum(lsr_value), .groups="drop") %>% ungroup() %>% as.data.frame() %>% select(Hypothesis, vctr.ctrl.eff, sens, spec, cum_lsr) %>% pivot_wider(names_from = spec, values_from=c(cum_lsr)) %>% group_by(sens) %>% mutate(across(3:5, ~ ifelse(. == min(.), text_spec(round(., 2), bold=TRUE), round(., 2)))) %>% arrange(sens) %>% kable('html', escape=FALSE) %>% kable_classic() #%>% row_spec(c(9), extra_css="border-bottom: 1px solid;")
}
...
data_manip_resid <- function(data, sens_spec) {
  data$probs <- runif(dim(data)[1])
  return(
    data %>% group_by(run_num, vctr.ctrl.eff) %>% mutate(
    ...
    ) %>% group_by(run_num, vctr.ctrl.eff, age_groups_lsr, Hypothesis) %>%
    summarise(
      ov16_prev = mean(ov16_prev),
      .groups="drop"
    ) %>% rename(age_groups = age_groups_lsr)
  ) 
}
custom_calc_resid <- function(original_data, new_data, desc) {
  ...
  lsr_calculation <- merged_data %>%
    group_by(Hypothesis, vctr.ctrl.eff, run_num, age_groups) %>%
    mutate(
      mse_value = (
        ((ov16_prev*100) - (ov16_seroprev*100))**2 *
          pct_pop
        )) %>%
    select(Hypothesis, vctr.ctrl.eff, run_num, age_groups, mse_value) %>%
    mutate(Descriptor = desc) %>% 
    group_by(Hypothesis, vctr.ctrl.eff, run_num, Descriptor) %>%
...
for(sens_spec in sens_spec_options) {
  print(sens_spec)
  new_result <- rbind(
      custom_calc_resid(togoOv16Data %>% filter(age_groups > 0), data_manip_resid(combinedSimulation_k3_100_mount, sens_spec), "No Seroreversion"),
      custom_calc_resid(togoOv16Data %>% filter(age_groups > 0), data_manip_resid(combinedSimulation_k3_finite_seroreversion %>% filter(), sens_spec), "Finite Seroreversion"),
      custom_calc_resid(togoOv16Data %>% filter(age_groups > 0), data_manip_resid(combinedSimulation_k3_instant_seroreversion, sens_spec), "Instant Seroreversion")
  )
...
process_resids <- function(resids, display_cis) {
  final_resids <- resids %>%
    mutate(
      Descriptor = factor(Descriptor, levels=c("No Seroreversion", "Midpoint Seroreversion", "Instant Seroreversion", "Finite Seroreversion"))
    ) %>%
    group_by(Descriptor, Hypothesis, vctr.ctrl.eff, sens_spec) %>%
    summarise(
  ...
  return(
    final_resids %>% 
      select(Descriptor, Hypothesis, vctr.ctrl.eff, spec, sens, output_val) %>%
      pivot_wider(id_cols=c(spec, sens, Hypothesis, vctr.ctrl.eff), names_from=c(Descriptor), values_from=c(output_val)) %>% group_by(spec, sens, Hypothesis) %>% 
      # mutate(
      #   across(2:4, ~ {
      ...
      # ) %>% 
      ungroup() %>% 
      arrange(desc(spec), sens, Hypothesis, vctr.ctrl.eff) %>% kable('html', escape=FALSE) %>% kable_classic() %>% print())
}

process_resids(resids, FALSE)
process_resids(resids, TRUE)
```
...
if (use_calc_lsr_pref) {
  data_manip_resid_pref <- function(data, sens_spec, prefecture_name) {
    ...
    return(
      ...
      group_by(run_num, vctr.ctrl.eff, Hypothesis) %>%
      summarise(
        ov16_prev = mean(ov16_prev),
        .groups="drop"
      ) %>%
  ...
  process_resids_pref <- function(resids, display_cis) {
    final_resids <- resids %>%
    ...
    return(
      final_resids %>% 
        ...
        select(prefecture, Descriptor, Hypothesis, vctr.ctrl.eff, spec, sens, output_val) %>%
        filter(
              (prefecture == "oti/kpendjal" & vctr.ctrl.eff==0.75) |
              (prefecture == "keran" & vctr.ctrl.eff==0.90) |
              (prefecture == "bassar" & vctr.ctrl.eff==0.60)
        ...
        mutate(
          across(-c(sens, spec), ~ {
            first_nums <- as.numeric(str_extract(.x, "^[0-9]+(\\.[0-9]+)?"))
            print(first_nums)
            min_val <- min(first_nums, na.rm = TRUE)
            print(min_val)
            ifelse(!is.na(first_nums) & first_nums == min_val,
                   text_spec(as.character(.x), bold=TRUE), 
                   .x)
          })
        ) %>% 
        ungroup() %>% 
        select(spec, sens, Hypothesis, `oti/kpendjal_No Seroreversion`, `oti/kpendjal_Finite Seroreversion`, `keran_No Seroreversion`, `keran_Finite Seroreversion`, `bassar_No Seroreversion`, `bassar_Finite Seroreversion`) %>%
        arrange(desc(spec), sens, Hypothesis) %>% kable('html', escape=FALSE) %>% kable_classic() %>% print())
  }
  
  process_resids_pref(resids_pref, FALSE)
  process_resids_pref(resids_pref, TRUE)
}
...
makeAgeGroupGraphsSensSpec(togoOv16Data, k3_100_mount_simulation_summary_age_group_combined_sens_spec_list, paste0("basic"), "sens ~ spec", "Sensitivity", "Specificity")

makeAgeGroupGraphsSensSpec(togoOv16Data, k3_instant_seroreversion_simulation_summary_age_group_combined_sens_spec_list, paste0("instant_seroreversion"), "sens ~ spec", "Sensitivity", "Specificity")

makeAgeGroupGraphsSensSpec(togoOv16Data, k3_gradual_seroreversion_simulation_summary_age_group_combined_sens_spec_list, paste0("gradual_seroreversion"), "sens ~ spec", "Sensitivity", "Specificity")
if (use_k4) {
...
```{r}
makePrefectureBarSensSpecBestFit <- function(original_data, projection_df_list, use_all_age=TRUE, descriptor="", sero_dfs=list()) {
  ylab <- "all ages"
  if (grepl("under10", descriptor)) {
    ylab <- "under 10 year-old"
  }

  pattern_vals <- c("Observed"="none", "H4i"="none", "H4iii"="stripe")
  fill_vals <- c("Observed"="grey", "H4i"="white", "H4iii"="white")
  ...
  combined_df_2 <- combined_df %>% 
    ...
    mutate(
      ...
      error_bar_colors = ifelse(vctr.ctrl.eff < 0, "Observed Error Bar", "Error Bar"),
      seroreversion = ifelse(seroreversion, "H4iii", "H4i"),
      seroreversion = ifelse(vctr.ctrl.eff < 0, "Observed", seroreversion),
      seroreversion = factor(seroreversion, levels=c("Observed", "H4i", "H4iii")),
      sens = paste(as.numeric(sens)*100, "%", sep=""),
      spec = paste(as.numeric(spec)*100, "%", sep="")
    )
...
makePrefectureBarSensSpecBestFit(groupedVillageData, k3_100_mount_simulation_summary_combined_sens_spec_list, TRUE, "finite_all_ages_best_vc", k3_gradual_seroreversion_simulation_summary_combined_sens_spec_list)

makePrefectureBarSensSpecBestFit(groupedVillageData, k3_100_mount_simulation_summary_combined_sens_spec_list, FALSE, "finite_under_10_best_vc", k3_gradual_seroreversion_simulation_summary_combined_sens_spec_list)
...
makeAgeGroupGraphsSensSpecBestFit <- function(originalData, projection_df_list, descriptor, facet_value="sens ~ spec", facet_1, facet_2) {
  originalData <- originalData %>% filter(age_groups != 0.0)
  combined_df <- NA
  for(sens_spec_val in names(projection_df_list)) {
    if(!is.data.frame(combined_df)) {
  ...
  tmp_projection_data <- combined_df_2 %>% 
    ...
    linetype_vals <- c("H4i"="solid", "H4ii"="dotted", "Midpoint"="dotted", "H4iii"="dashed")
    p1 <- ggplot() + 
      geom_line(
        aes(x=age_groups, y=ov16_prev*100, 
            linetype=sero_type,
      ...
      xlab("Age (years)") + ylab("Anti-Ov16 seroprevalence (%)") +
      scale_color_manual("Hypothesis", values=color_vals) +
      scale_fill_manual("Hypothesis", values=color_vals) +
      scale_linetype_manual("Hypothesis", values=linetype_vals) +
      theme_bw() +
      facet_grid(facet_value) +
      scale_y_continuous(
        limits=c(0, 101), 
...
for (sens_spec_val in sens_spec_options) {
  print(sens_spec_val)
  tmp <- summarizeSimulationData(best_fit_total, sens_spec_val, groupBySex=FALSE) %>% mutate(sero_type=ifelse(vctr.ctrl.eff == 0.6, "H4i", "H4iii"))
  tmp_hi <- tmp %>% filter(sero_type == "H4i")
  tmp_hiii <- tmp %>% filter(sero_type == "H4iii") %>% select(age_groups, Hypothesis, ov16_prev)
  tmp_midpoint <- tmp_hi %>% left_join(tmp_hiii, by=c("age_groups", "Hypothesis")) %>% mutate(ov16_prev = (ov16_prev.x + ov16_prev.y) / 2, sero_type="Midpoint") %>% select(-ov16_prev.x, -ov16_prev.y) %>% mutate(
...
makeAgeGroupGraphsSensSpecBestFit(togoOv16Data, best_fit_vc, "best_vc", "~sens_spec", "", "Sensitivity, Specificity")
