...
create_mfp_graph <- function(actual_data, projection_data, kE) {
    ...
    mfp_plot <- mfp_plot +
      ...
      geom_text(
        data = data.frame(list(
          exposure = c("EPIONCHO exposure", "ONCHOSIM exposure"),
          label = c("a", "b"),
          x = c(-Inf, -Inf),
          y = c(Inf, Inf)
        )),
        aes(x=x, y=y, label=label),
        hjust = 1.5, vjust = -0.65,
        size = 7 / .pt,
        fontface = "bold"
      )
    width_val <- 4750
  ...
  ggsave(paste0("images/images_", output_version,"/gabon_rdt_mfp_graph_new_quartiles_", kE, ".pdf"),mfp_plot, width=175, height=110.5, units="mm", dpi=450)
}
...
if (use_onchosim) {
  ...
  scale_fill_gradientn(
    ...
    ggsave(
      paste0("images/images_", output_version, "/k2_normalized_mses_3_sigfigs.pdf"),
      normalized_mse_plot,
...
#### Figure 4, with `use_onchosim = TRUE`
...
ggsave(paste0("images/images_", output_version,"/best_fitting_hyp_using_sampled_diagnostic_performance.pdf"), plot_1, width=175, height=110.5, units="mm", dpi=600)

#### Supplementary Figure 5 with `use_onchosim = TRUE`
...
#### Supplementary Figures 6 - 8, done with `use_onchosim = FALSE`

supp_sero_hyp <- all_sero_hyp %>% 
  filter(
    sens %in% c("40%", "50%", "60%"),
    exposure == "EPIONCHO exposure"
  ) %>%
  mutate(
    sero_hyp = case_when(
      grepl("iii", Hypothesis) ~ "(iii)",
...
# H1-3/H3 = Blue, H1-2 = Purple, H4 = Red
for (sero_hyp_val in unique(supp_sero_hyp$sero_hyp)) {
  print(sero_hyp_val)
  tmp_supp_sero_hyp <- supp_sero_hyp %>%
    filter(sero_hyp == sero_hyp_val) %>%
    mutate(
      ov16_prev = ifelse(age_groups == 0, 0, ov16_prev),
      ov16_lower = ifelse(age_groups == 0, 0, ov16_lower),
      ov16_upper =  ifelse(age_groups == 0, 0, ov16_upper)
    )
  sero_hyp_plot <- ggplot() + 
    geom_point(aes(x=age_groups, y=ov16_prev_rdt*100), alpha=0.5, color="black", size=0.5, data=df1 %>% filter(age_groups > 0)) +
      geom_errorbar(aes(x=age_groups, ymin=(ov16_wilson_lb*100), ymax=(ov16_wilson_ub*100)), width=2.5, alpha=0.5, colour="black", data=df1 %>% filter(age_groups > 0)) +
     geom_line(aes(x=age_groups, y=ov16_prev*100, color=Hypothesis), linewidth=0.6, data=tmp_supp_sero_hyp) +
      geom_ribbon(aes(x=age_groups, ymin=ov16_lower*100, ymax=ov16_upper*100, fill=Hypothesis), color=NA, alpha=0.1, data=tmp_supp_sero_hyp) +
    scale_fill_manual(name="Hypotheses", values=c("H3ii"="blue", "H1ii-2ii"="purple", "H1i-3i"="blue", "H1iii-3iii"="blue", "H4i"="red", "H4ii"="red", "H4iii"="red")) +
    scale_color_manual(name="Hypotheses", values=c("H3ii"="blue", "H1ii-2ii"="purple", "H1i-3i"="blue", "H1iii-3iii"="blue", "H4i"="red", "H4ii"="red", "H4iii"="red")) +
    scale_alpha_continuous(range = c(0.3, 1)) +
    theme_bw() +
  ...
  ggsave(paste0("images/images_", output_version,"/point_estimate_diagnostic_seroreversion", sero_hyp_val, ".svg"), sero_hyp_plot, width=4250, height=3000, units="px", dpi=600)
}
