...
new_cases_morbidity_func <- function(morb.mat.tmp, SI_probs, RSD_probs, Atrp_probs, Hg_probs, Depigm_probs){
  ...
  morb.mat.tmp$AtrophyStatus <- ifelse(morb.mat.tmp$AtrophyStatus == 0, rbinom(length(Atrp_probs), 1, calc_daily_prob(Atrp_probs, 365)), morb.mat.tmp$AtrophyStatus) # atrophy (non-reversible: only testing 0's
  morb.mat.tmp$HGStatus <- ifelse(morb.mat.tmp$HGStatus == 0, rbinom(length(Hg_probs), 1,  calc_daily_prob(Hg_probs, 365)), morb.mat.tmp$HGStatus) # HG (non-reversible: only testing 0's
  morb.mat.tmp$DepigStatus <- ifelse(morb.mat.tmp$DepigStatus == 0, rbinom(length(Depigm_probs), 1,  calc_daily_prob(Depigm_probs, 365)), morb.mat.tmp$DepigStatus) # depigmentation (non-reversible: only testing 0's
